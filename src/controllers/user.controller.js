...
const loginUser = asyncHandler(async(req, res)=>{
    ...
    // if not exist ask user to register
    // if user exist verify its password
    // if password is wrong give error to enter correct password
    // if password is correct then generate token 
    // send cookie with response
     const {email, username, password} = req.body;
     if(!(email || username) || !password) {
        throw new ApiError(403, "email password is required");
     }
...
const updateUserDetails = asyncHandler(async (req, res) => {
    ...
    if(!email || !fullName){
        throw new ApiError(400, "email or full name required")
    }
...
const updateUserAvatar = asyncHandler(async (req, res) => {
     ...
     const avatar = await uploadOnCloudinary(avatarLocalPath);
     if(!avatar.url){
        throw new ApiError(400, "Erorr while uploading avatar file");
     }
